...
class DropdownSkillsSelection extends React.Component {
  ...
  render() {
    const options = this.state.skills.map(skill => { return { key: skill.id, text: skill.name, value: skill.name } });
    return <Dropdown placeholder="Filter by skills" fluid multiple selection options={options}/>
  }
};
